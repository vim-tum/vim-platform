...
class RTXDefinition:
    ...
    def __init__(self, oeda_experiment, oeda_target, oeda_callback, oeda_stop_request):
        ...
        self.incoming_data_types = oeda_target[
            "incomingDataTypes"]  # contains all of the data types provided by both config & user
    ...
    def is_type(self, experiment_type):
        return experiment_type in self._oeda_target["type"]

    def get_channel(self, channel):
        if self.primary_data_provider["name"] == channel:
            return self.primary_data_provider["instance"]
        for secondary_data_provider in self.secondary_data_providers:
            if secondary_data_provider["name"] == channel:
                return secondary_data_provider["instance"]

    def save_experiment_result_url(self, result_url):
        print("Printing result url in rtx def file")
        print(result_url)
        db().update_experiment(self.id, "simulation", {
            "result_url": result_url})

    def update_experiment_status(self, new_experiment_status):
        db().update_experiment(self.id, "status", new_experiment_status)

    def update_targetSystem_status(self, new_targetSystem_status):
        db().update_target_system(self._oeda_target["id"], "status", new_targetSystem_status)
    ...
    def primary_data_reducer(new_data, wf):
        db().save_data_point(experiment_id=wf.id, step_no=wf.step_no, stage_no=wf.stage_counter,
                             data_point_count=wf.primary_data_counter, secondary_data_provider_index=None,
                             payload=new_data)
    ...
    def secondary_data_reducer(new_data, wf, idx):
        db().save_data_point(experiment_id=wf.id, step_no=wf.step_no, stage_no=wf.stage_counter,
                             data_point_count=wf.secondary_data_counter, secondary_data_provider_index=idx,
                             payload=new_data)
    ...
    def setup_stage(wf, knobs):
        db().save_stage(experiment_id=wf.id, step_no=wf.step_no, step_name=wf.step_name, stage_no=wf.stage_counter,
                        knobs=knobs)
    ...
    def evaluator(wf):
        ...
        db().update_stage(experiment_id=wf.id, step_no=wf.step_no, stage_no=wf.stage_counter, field="stage_result",
                          value=result)
