...
import { AnalysisGroupViewComponent } from "../../../../shared/modules/ui/analysis-group-view.component";
import { AnalysisModuleComponent } from "../../../../shared/modules/ui/analysis-module.component";
...
export class ShowRunningExperimentComponent implements OnInit, OnDestroy {
  ...
  @ViewChildren(AnalysisGroupViewComponent)
  analysisGroupView: QueryList<AnalysisGroupViewComponent>;

  @ViewChild("analysisModule")
  analysisModule: AnalysisModuleComponent;
  ...
  private fetch_oeda_callback() {
    ...
    if (document.getElementById("polling_on_button")) {
      document
        .getElementById("polling_on_button")
        .setAttribute("disabled", "true");
    }

    ctrl.apiService
      .getOedaCallback(ctrl.experiment_id)
      .subscribe((oedaCallback) => {
        if (!isNullOrUndefined(oedaCallback)) {
          ...
          if (oedaCallback.status === "PROCESSING") {
            ...
            oedaCallback.status == "IGNORING_SAMPLES" ||
            oedaCallback.status == "COLLECTING_DATA"
          ) {
            ctrl.oedaCallback["index"] = oedaCallback.index;
            ctrl.oedaCallback["size"] = oedaCallback.size;
            ctrl.oedaCallback["complete"] =
              Number(oedaCallback.index) / Number(oedaCallback.size);

            // round knob values to provided decimal places
            oedaCallback.current_knob = ctrl.entityService.round_values(
              oedaCallback.current_knob,
              ctrl.decimal_places
            );
            // remaining_time_and_stages is a experiment-wise unique dict that contains remaining stages and time
            ctrl.oedaCallback.remaining_time_and_stages["remaining_time"] =
              oedaCallback.remaining_time_and_stages["remaining_time"];
            ctrl.oedaCallback.remaining_time_and_stages["remaining_stages"] =
            ...
            if (oedaCallback.status == "COLLECTING_DATA") {
              if (ctrl.first_render_of_page) {
                ctrl.apiService
                  .loadAllDataPointsOfExperiment(ctrl.experiment_id)
                  .subscribe((response) => {
                    ...
                    if (ctrl.incoming_data_type == null) {
                      ...
                      ctrl.selected_stage =
                        ctrl.available_steps[ctrl.step_no]["stages"][0];
                      let first_stage_data =
                        ctrl.available_steps[ctrl.step_no]["stages"][1];
                      ctrl.incoming_data_type = ctrl.entityService.get_candidate_data_type(
                        ctrl.experiment,
                        ctrl.targetSystem,
                        first_stage_data
                      );
                    ...
                    if (
                      is_successful_fetch &&
                      !isNullOrUndefined(ctrl.incoming_data_type)
                    ) {
          ...
          } else if (oedaCallback.status == "COLLECTING_ANALYSIS_DATA") {
            this.prepareAnalysis(oedaCallback);
            ...
            this.apiService
              .updateExperiment(this.experiment)
              .subscribe((resp) => {
                ctrl.disable_polling(
                  "Success",
                  "Data is up-to-date, stopped polling."
                );
    ...
    if (ctrl.first_render_of_page) {
      ...
      for (let step_no in steps) {
        if (steps.hasOwnProperty(step_no)) {
          ...
          all_stages_tpl.knobs = ctrl.entityService.populate_knob_objects_with_variables(
            all_stages_tpl.knobs,
            ctrl.targetSystem.defaultVariables,
            true
          );
          ...
          for (let stage_no in steps[step_no]) {
            if (steps[step_no].hasOwnProperty(stage_no)) {
              ...
              if (this.get_keys(stage_object).length !== 0) {
                ...
                stage_entity.knobs = ctrl.entityService.round_values(
                  stage_object["knobs"],
                  ctrl.decimal_places
                );
                stage_entity.knobs = ctrl.entityService.populate_knob_objects_with_variables(
                  stage_entity.knobs,
                  ctrl.targetSystem.defaultVariables,
                  false
                );
          ...
          step_entity.stages.sort(
            this.entityService.sort_by("number", true, parseInt)
          );
      ...
      for (let step_no in steps) {
        if (steps.hasOwnProperty(step_no)) {
          ...
          for (let stage_no in steps[step_no]) {
            ...
            if (
              steps[step_no].hasOwnProperty(stage_no) &&
              this.get_keys(steps[step_no][stage_no]).length != 0
            ) {
              let stage_object = steps[step_no][stage_no];
              // find the retrieved stage in available stages
              let existing_stage = step.stages.find(
                (entity) => entity["number"] === stage_object.number
              );
              ...
              if (existing_stage !== undefined) {
                ...
                if (stage_object["values"].length > 0) {
                  existing_stage["values"] = existing_stage["values"].concat(
                    stage_object["values"]
                  );
                }
                ...
                let knobs = ctrl.entityService.round_values(
                  stage_object["knobs"],
                  ctrl.decimal_places
                );
                knobs = ctrl.entityService.populate_knob_objects_with_variables(
                  knobs,
                  ctrl.targetSystem.defaultVariables,
                  false
                );
              ...
              if (Number(stage_no) == last_stage_idx) {
                ...
                if (data_point_length > 0) {
                  ctrl.timestamp =
                    stage_object.values[data_point_length - 1]["createdDate"];
  ...
  private draw_all_plots() {
    ...
    if (ctrl.selected_stage.number == -1) {
      ctrl.processedData = ctrl.entityService.process_stages(
        ctrl.available_steps[ctrl.step_no].stages,
        "timestamp",
        "value",
        ctrl.scale,
    ...
    // if any other stage is selected
      ...
      ctrl.processedData =
        ctrl.available_steps[ctrl.step_no].stages[ctrl.selected_stage.number];
      ctrl.processedData = ctrl.entityService.process_single_stage_data(
        ctrl.processedData,
        "timestamp",
        "value",
        ctrl.scale,
    ...
    if (!isNullOrUndefined(ctrl.processedData)) {
      ...
      ctrl.initial_threshold_for_scatter_plot = ctrl.plotService.calculate_threshold_for_given_percentile(
        clonedData,
        95,
        "value",
        ctrl.decimal_places
      ...
      ctrl.nr_points_to_be_filtered = ctrl.processedData.filter(function (
        item
      ) {
      ...
      if (ctrl.first_render_of_plots) {
        ctrl.scatter_plot = ctrl.plotService.draw_scatter_plot(
          ctrl.divId,
          ctrl.filterSummaryId,
          ctrl.processedData,
          ctrl.incoming_data_type["name"],
          ctrl.initial_threshold_for_scatter_plot,
          ctrl.stage_details,
          ctrl.decimal_places
        );
        ...
        console.log(ctrl.processedData);
        ctrl.scatter_plot.graphs[0].negativeBase =
          ctrl.initial_threshold_for_scatter_plot;
        ctrl.scatter_plot.valueAxes[0].guides[0].value =
          ctrl.initial_threshold_for_scatter_plot;
        ...
        ctrl.histogram.dataProvider = ctrl.plotService.categorize_data(
          ctrl.processedData,
          ctrl.decimal_places
        );
      ...
      ctrl.is_enough_data_for_plots = false;
      this.notify.error(
        "Cannot retrieve data from server, please restart experiment"
      );
    }
  }
  ...
  stage_changed(selected_stage) {
    ...
    if (!isNullOrUndefined(this.incoming_data_type)) {
      ...
      this.is_enough_data_for_plots = false;
      this.notify.error(
        "Incoming data type is null, select it from dropdown list"
      );
    }
  }
  ...
  is_considered(data_type_name) {
    return this.entityService.is_considered(
      this.experiment.considered_data_types,
      data_type_name
    );
  }
  ...
  disable_polling(status: string, content: string) {
    document
      .getElementById("polling_off_button")
      .setAttribute("class", "btn btn-primary active");
    document
      .getElementById("polling_off_button")
      .setAttribute("disabled", "true");
    document
  ...
  private prepareAnalysis(oedaCallback: any) {
    if (!isNullOrUndefined(this.oedaCallback["algorithms"])) {
      const toUpdate = [];
      oedaCallback.algorithms
        .filter((algorithm, index) => {
          const update =
            this.oedaCallback["algorithms"][index]["results"]["version"] <
            algorithm["results"]["version"];
          console.log("Update required: " + update);
          return update;
        })
        .forEach((algorithm) => toUpdate.push(algorithm.name));
      this.analysisModule.updateAnalysisResults(toUpdate);
    }
    this.oedaCallback = oedaCallback;
    /*
    this.oedaCallback["algorithms"].filter((algorithm) => algorithm['updateRequired']).forEach((algorithm) => {
      const found = this.analysisGroupView.find((analysis) => analysis.algorithmName === algorithm.name);
      found.fetchAnalysisResults();
    });
    */
  }
